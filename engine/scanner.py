...
from patterns.double_bottom import detect_double_bottom as detect_double_bottom_tuned
...
def scan_stock(symbol):
    ...
    result = (
        detect_cup_handle_monthly(df_monthly) or  # Monthly C&H — multi-year bases
        detect_cup_handle_weekly(df_weekly) or    # Weekly C&H
        detect_cup_handle(df_daily) or            # Daily C&H (tuned — diagonal neckline)
        detect_double_bottom_tuned(df_daily) or   # Tuned double bottom (full measured move)
        detect_double_bottom(df_daily) or         # Original fallback
        detect_double_top(df_daily) or            # Short setup
        detect_descending_wedge(df_daily) or      # Descending wedge breakout
    ...
    if result["pattern"] in ("Cup & Handle (Monthly)", "Cup & Handle (Weekly)"):
        bonus += 30  # Multi-TF cup — highest conviction
    elif result["pattern"] == "Cup & Handle":
        bonus += 25  # Very reliable
    elif result["pattern"] == "Double Bottom":
        bonus += 20  # Strong reversal
    elif result["pattern"] == "Descending Wedge":
        bonus += 14  # Bullish reversal
